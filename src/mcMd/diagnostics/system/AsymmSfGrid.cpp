#ifndef MCMD_ASYMM_SF_GRID_CPP
#define MCMD_ASYMM_SF_GRID_CPP
...
namespace McMd
{
   ...
   * Constructor.
   */
   AsymmSfGrid::AsymmSfGrid(System& system) 
    : AsymmSf(system),
      hMax_(0),
      nStar_(0),
      lattice_(Triclinic),
   ...
   */
   void AsymmSfGrid::loadParameters(Serializable::IArchive& ar)
   {
      // Load from AsymmSf::serialize
      ...
      // Load additional from AsymmSfGrid::serialize
      ...
      // Allocate temporary data structures (defined in AsymmSf)
      waveVectors_.allocate(nWave_);
      fourierModes_.allocate(nWave_, nMode_);
   ...
   */
   void AsymmSfGrid::save(Serializable::OArchive& ar)
   {  ar & *this; }

   void AsymmSfGrid::setup() 
   {}

   void AsymmSfGrid::sample(long iStep)
   {
      AsymmSf::sample(iStep);
